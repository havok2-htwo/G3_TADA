...
class TadaForCausalLM(LlamaForCausalLM):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *args, **kwargs):
        ...
        model_source, model_is_local = resolve_pretrained_source(str(pretrained_model_name_or_path), project_root)
        load_kwargs = dict(kwargs)
        if model_is_local:
            load_kwargs["local_files_only"] = True

        self = super().from_pretrained(model_source, *args, **load_kwargs)
        # Forward memory/dtype options to the separately loaded codec decoder as well.
        sub_kwargs = {k: v for k, v in load_kwargs.items() if k in ("low_cpu_mem_usage", "local_files_only", "dtype", "torch_dtype")}
        decoder_source, _ = resolve_pretrained_source("HumeAI/tada-codec", project_root)
        self._decoder = Decoder.from_pretrained(decoder_source, subfolder="decoder", **sub_kwargs)
        ...
        tokenizer_source, tokenizer_is_local = resolve_pretrained_source(tokenizer_name, project_root)
        tokenizer_kwargs = {"local_files_only": True} if tokenizer_is_local else {}
        self._tokenizer = AutoTokenizer.from_pretrained(tokenizer_source, **tokenizer_kwargs)
        return self
    @property
    ...
    def _build_time_schedule(num_steps: int, schedule: str, device: torch.device) -> torch.Tensor:
        ...
        """
        if schedule == "cosine":
            # t = 0.5 * (1 - cos(?? * u))  where u is uniform in [0, 1]
        ...
        if schedule == "logsnr":
            ...
            # and map back to t.  t = ??(???log_snr/2) where ?? is the sigmoid.
    ...
    def _generate(
        ...
        **kwargs,
        ...
        if not use_text_in_prompt:
            ...
            max_t = min(prompt_token_len, input_ids.shape[1])
            keep_mask = torch.zeros(input_ids.shape[0], max_t, dtype=torch.bool, device=input_ids.device)
            ...
            input_ids[:, :max_t][~keep_mask] = pad_id
        ...
        def emit_progress(step_index: int, is_final: bool = False) -> None:
            if progress_callback is None:
                return
            if not is_final and progress_interval_steps > 1 and (step_index + 1) % progress_interval_steps != 0:
                return
            if len(all_acoustic_features) == 0:
                return

            acoustic_items = [f if f.ndim == 3 else f.unsqueeze(1) for f in all_acoustic_features]
            time_items = [f if f.ndim == 2 else f.unsqueeze(1) for f in all_time_before]
            if last_time_before is not None and len(time_items) == len(acoustic_items):
                time_items.append(last_time_before if last_time_before.ndim == 2 else last_time_before.unsqueeze(1))
            if len(time_items) != len(acoustic_items) + 1:
                return
            ...
            progress_callback(
                {
                    "step": step_index,
                    "is_final": is_final,
                    "acoustic_features": torch.cat(acoustic_items, dim=1),
                    "time_before": torch.cat(time_items, dim=1),
                }
            )
            ...
            step_logs.append(
                {
                    ...
                    "n_frames_before": time_len_before[0].reshape(-1)[0].item(),
                    "n_frames_after": time_len_after[0].reshape(-1)[0].item(),
                    "n_frames_src": time_len_type,
                    "acoustic_mask": acoustic_masks[0].reshape(-1)[0].item(),
        ...
        if last_time_before is not None:
            ...
            emit_progress(num_steps - 1, is_final=True)
    ...
    def generate(  # type: ignore[override]
        ...
        input_ids = [
            self.tokenizer.encode(prompt.text[i] + text[i])[int(prompt.text_tokens_len[i].item()) :] for i in range(len(text))
        ]
        audio_feat_len = (prompt.audio_len / prompt.sample_rate * 50).ceil().long()

        text_tokens = []
        for index, t in enumerate(text):
            # Use the EXACT cached tokens for the prompt to prevent length mismatch with acoustic features
            if prompt.text_tokens is not None and prompt.text_tokens_len is not None:
                prompt_tokens_list = prompt.text_tokens[index, : int(prompt.text_tokens_len[index].item())].tolist()
            else:
                prompt_tokens_list = self.tokenizer.encode(prompt.text[index], add_special_tokens=False)
            # Re-encode the newly provided text without special tokens
            new_text_tokens = self.tokenizer.encode(t, add_special_tokens=False)
            text_tokens.append(prompt_tokens_list + new_text_tokens)
        
        # Pad sequences to max length, ensuring we ONLY use EOS for padding!
        # If we use pad_token_id, the Flow Vocoder goes insane and predicts GrayCode ~1023 (20s of silence)
        text_tokens = [t + [self.eos_id] * self.num_eos_tokens for t in text_tokens]
        max_len = max(len(t) for t in text_tokens)
        padded_tokens = [t + [self.eos_id] * (max_len - len(t)) for t in text_tokens]
        ...
        batch_size = input_ids.shape[0]
        prefix_text_tokens = prefix_text_tokens.expand(batch_size, -1)
            ...
            progress_callback(
                {
                    ...
                    "encoded": encoded,
                    "time_before": stream_time_before,
                    "input_lengths": input_lengths,
                    "stream_offset": stream_offset,
                }
            )
            
        # Expand prompt tensors to match the actual calculated batch size
        batch_size = input_ids.shape[0]
        if prompt_acoustic_features is not None and prompt_acoustic_features.shape[0] == 1 and batch_size > 1:
            prompt_acoustic_features = prompt_acoustic_features.expand(batch_size, -1, -1)
        if prompt_acoustic_masks is not None and prompt_acoustic_masks.shape[0] == 1 and batch_size > 1:
            prompt_acoustic_masks = prompt_acoustic_masks.expand(batch_size, -1)
        if time_len_before is not None and time_len_before.shape[0] == 1 and batch_size > 1:
            time_len_before = time_len_before.expand(batch_size, -1)
        if time_len_after is not None and time_len_after.shape[0] == 1 and batch_size > 1:
            time_len_after = time_len_after.expand(batch_size, -1)
        ...
        for i in range(encoded.shape[0]):
            try:
                ...
                if fade_len > 0:
                    fade_curve = torch.linspace(0.0, 1.0, fade_len, device=wav.device, dtype=wav.dtype)
    ...
    def _add_bos_eos(self, input_ids: torch.Tensor, input_lengths: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        # input_ids is already padded with eos_id properly from the caller text_tokens logic.
        # We just need to prepend bos_id.
