...
class LocalSelfAttention(torch.nn.Module):
    ...
    def _apply_rope(self, x: torch.Tensor, seq_len: int) -> torch.Tensor:
        ...
        freqs_cos = freqs[..., 0].to(x.dtype)  # (seq_len, head_dim // 2)
        freqs_sin = freqs[..., 1].to(x.dtype)  # (seq_len, head_dim // 2)
    ...
    def forward(self, x: torch.Tensor, mask: torch.Tensor | None = None) -> torch.Tensor:
        ...
        # Apply attention to values in the same dtype to avoid bf16/float32 matmul mismatches.
